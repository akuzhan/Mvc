...
namespace Microsoft.AspNet.Mvc
{
    public class SerializableErrorTests
    {
        ...
        public void ConvertsModelState_To_Dictionary_AddsDefaultValuesWhenErrorsAreAbsent()
        {
            ...
            Assert.NotNull(serializableError);
            var arr = Assert.IsType<string[]>(serializableError["key1"]);
            Assert.Equal("An error has occured.", arr[0]);
        }

        [Fact]
        public void ThrowsOnValidModelState()
        {
            // Arrange, Act & Assert
            var exception = Assert.Throws<InvalidOperationException>(() => {
                new SerializableError(new ModelStateDictionary());
                });
            Assert.Equal("The model state is valid.", exception.Message);
        }
        ...
        [Fact]
        public void ReadXml_ReadsSerailizableErrorXml()
        {
            // Arrange
